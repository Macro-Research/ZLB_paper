...
for t=2:N
 ...
 regime(t)=findRegime(regime(t-1),p_LN,p_NL);
...
M=5000;
dataset=X(1:3,:)';size_dataset=size(dataset);l=size_dataset(2);
T= size(dataset,1);
lik=zeros(T,1);
...
prob_regime(:,1)=[1-regime(1) regime(1)];
...
xx=['LOG LIKL:',num2str((-sum(log(lik(2:end)))))];disp(xx);
...
for jj=1:numVar
subplot(numVar,1,jj)
plot(filtered_states(:,jj),'*');hold on;plot(X(jj,:),'lineWidth',3);
...
save simulated_dataset.mat dataset;