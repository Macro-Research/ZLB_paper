...
filtered_states=s_up1.*repmat(prob_update(1,:)',[1 numVar]) +...
    s_up2.*repmat(prob_update(2,:)',[1 numVar]);
...
% subplot(numVar,1,jj)
    if jj==3;
plot(filtered_states(:,jj)+prob_update(1,:)'*param1(jj)+prob_update(2,:)'*param2(jj)...
    ,'*');hold on;plot(dataset(:,jj),'lineWidth',3);
    else
    plot(filtered_states(:,jj)+param1(jj)...
    ,'*');hold on;plot(dataset(:,jj),'lineWidth',3);
...
plot(prob_update(2,:),'*');
hold on;
plot(prob_update(2,:),'lineWidth',2);
legend('filtered regimes probabilities');
