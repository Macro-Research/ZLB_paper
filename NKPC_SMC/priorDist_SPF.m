function [prior]=priorDist_SPF(param)

%normal regime parameters
prior1=normpdf(param(1),0,0.2);
% mu=0.62;sigma2=0.15^2;b  = sigma2/mu;a  = mu/b;
prior2=normpdf(param(2),0,0.2);
% mu=0.5;sigma2=0.15^2;b  = sigma2/mu;a  = mu/b;
prior3=normpdf(param(3),0,0.2);
mu=0.1;sigma2=0.02^2;a = (1-mu)*mu*mu/sigma2-mu;b = a*(1/mu-1);
...
prior17=exp(lnpdfig(param(17),0.07,2));
prior18=exp(lnpdfig(param(18),0.03,2));
...
prior=prior1*prior2*prior3*prior4*prior5*prior6*prior7*...
    ...
    prior14*prior15*prior16*prior17*prior18*prior19*prior20*prior21;
 prior=-log(prior);
end