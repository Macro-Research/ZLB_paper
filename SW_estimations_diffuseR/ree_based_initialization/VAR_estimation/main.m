...
% init_H1=nearestSPD(inv(diag(diag(init_H1))));
 ...
 %load('init_H1.mat');
 init_H_fullSample=nhess_diagonal(@likelihood,param_init);
 init_H_fullSample=inv(init_H_fullSample);init_H_fullSample=diag(diag(init_H_fullSample));
 save init_H_fullSample.mat init_H_fullSample;
 %load('init_H_fullSample.mat')
[fh,x,gh,H,itct,fcount,retcodeh] = csminwel('likelihood',param_init,init_H_fullSample,[] ,10^(-4),9999);% laplace_=laplace_approximator(fh,x,H);
