...
% rr_tt=(rr_tt+rr_tt')/2;
 ...
 %load param_init_alternative.mat;
...
%load('raf_dataset.mat');first_obs=47;last_obs=147;
...
hold on;
plot(S_filtered(2:end,12),'lineWidth',3);
fig = gcf;
...
%     index=index+1;
%     subplot(7,1,index);
%     plot(learning_filtered(:,1,jj),'lineWidth',3);
% end
plot(squeeze(learning_filtered(2:end,1,:)),'color','black');
fig = gcf;
fig.PaperPositionMode = 'auto'
fig_pos = fig.PaperPosition;
fig.PaperSize = [fig_pos(3) fig_pos(4)];
...
%     
% 
 figure('Name','MSV Learning Coef-Monetary Policy Shock','units','normalized','outerposition',[0 0 1 1]);
% 
index=0;
% for jj=1:7
%     index=index+1;
%     subplot(7,1,index);
%     plot(learning_filtered(5:end,13,jj),'lineWidth',3);
% end
plot(squeeze(learning_filtered(2:end,13,:)),'color','black');
fig = gcf;
fig.PaperPositionMode = 'auto'
fig_pos = fig.PaperPosition;
fig.PaperSize = [fig_pos(3) fig_pos(4)];
print(fig,'sw_msv_learning_mpShock','-dpdf'); 
%     
%   
%     
% 

 figure('Name','MSV Learning Coef-Lagged interest rate','units','normalized','outerposition',[0 0 1 1]);
% 
index=0;
% for jj=1:7
%     index=index+1;
...
%     plot(learning_filtered(5:end,13,jj),'lineWidth',3);
% end
plot(squeeze(learning_filtered(2:end,7,:)),'color','black');
fig = gcf;
fig.PaperPositionMode = 'auto'
fig_pos = fig.PaperPosition;
fig.PaperSize = [fig_pos(3) fig_pos(4)];
print(fig,'sw_msv_learning_laggedInt','-dpdf'); 
%     


 figure('Name','MSV Learning Coef-Projection Flag','units','normalized','outerposition',[0 0 1 1]);
% 
index=0;
subplot(3,1,1);
plot(pr_flag(2:end),'lineWidth',3);title('projection facility');
subplot(3,1,2);
plot(largest_eig1(2:end),'lineWidth',3);
hold on;
plot(ones(T-1,1),'--');title('largest eigenvalue, normal regime');
subplot(3,1,3);
plot(largest_eig2(2:end),'lineWidth',3);
hold on;
plot(ones(T-1,1),'--');title('largest eigenvalue, weighted average');

fig = gcf;
fig.PaperPositionMode = 'auto'
fig_pos = fig.PaperPosition;
fig.PaperSize = [fig_pos(3) fig_pos(4)];
print(fig,'sw_msv_pr_flag','-dpdf');    
...
%         index=index+1;;
%         subplot(7,1,index);
% plot(learning_filtered(5:end,1+jj,ii));
%     end
% end
...
% figure('Name','MSV Learning Coef-Intercepts','units','normalized','outerposition',[0 0 1 1]);
% for jj=1:length(forward_indices);
% subplot(length(forward_indices),1,jj);
% plot(learning_filtered(:,jj,1),'lineWidth',3);
% end
% fig = gcf;
% fig.PaperPositionMode = 'auto'
% fig_pos = fig.PaperPosition;
