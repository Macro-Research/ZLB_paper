...
N=5000;numVar=24;numShocks=7;numEndo=17;numExo=7;numBackward=7;numForward=7;
eig_crit=.99;
...
rr_tt=10*eye(numEndo+numExo+1,numEndo+numExo+1);
...
thetaOld=[alpha_tt(forward_indices)...
    beta_tt(forward_indices,:) cc_tt(forward_indices,:)]';

[theta,rr_tt ,largest_eig1(tt),largest_eig2(tt),pr_flag(tt)] =...
    msv_learning3(XX(forward_indices,tt),...
    [1;XX(1:numEndo,tt-1);XX(numEndo+1:end,tt)],...
    thetaOld,rr_old,gain,...
    forward_indices,backward_indices,numEndo,AA1,BB1,CC1,AA2,BB2,CC2,eig_crit,ergodic_states);

eig_max(tt)=max(largest_eig1(tt),largest_eig2(tt));
%-------------------------------update expectations

alpha_tt(forward_indices)=theta(1,:)';
beta_tt(forward_indices,:)=theta(2:numEndo+1,:)';
cc_tt(forward_indices,:)=theta(numEndo+2:end,:)';
