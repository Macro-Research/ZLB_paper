function [ AA BB CC DD EE RHO FF GG] = SW_matrixConverter( parameters )
...
err7=-eps_w + rho_w*eps_wm + eta_w - mu_w*eta_wm==0 ;
   ...
   equations_exo=[ err1,err2,err3,err4,err5,err6,err7];
     ...
     Contemp_endo = [mc,zcap,rk,k1,q,c,inve,y,lab,pinf,w,r,kp]; ...
     Contemp_exo= [eps_a,eps_b,eps_g,eps_i,eps_r,eps_p,eps_w ];
           
     Expectation_endo=[mcp zcapp rkp k1p   qp cp invep yp labp pinfp wp rp kpp];...
    
     
     Lagged_endo=[mcm zcapm rkm k1m   qm cm invem ym labm pinfm wm rm kpm];...
...
%         for j=1:NumEndo
%             
%             AA(m,j) = double(AA(m,j));
%             BB(m,j) = double(BB(m,j));
%             CC(m,j) = double(CC(m,j));
%             
%         end
%     end
%     
...
%         for j=1:NumExo
%             
%             DD(m,j) = double(DD(m,j));
%             EE(m,j) = double(EE(m,j));
%        end
%     end
%     
%     for m=1:NumExo
%         for j=1:NumExo
%             
%             RHO(m,j)=double(RHO(m,j));
%             FF(m,j)=double(FF(m,j));
%             GG(m,j)=double(GG(m,j));
%         end
%     end
