...
psi  phi lambda phi_p iota_w xi_w iota_p  xi_p sigma_c sigma_l r_b
r_pi r_dy r_y rho 
  beta 
...
// sticky price - wage economy
	      ...
	      r =  r_pi*(1-rho)*pinf
               ...
               +r_dy*(y-phi_p*eps_a-y(-1)+phi_p*eps_a(-1)+r_b*eps_b)
               +rho*r(-1)
               +eps_r  ;
...
estimation(filtered_vars,optim=('MaxIter',500),nograph,datafile=raf_dataset,mode_compute=0,mode_file=SW_Estimation_REE_mode,first_obs=71,presample=4,kalman_algo=1,lik_init=2,prefilter=0,mh_replic=0,mh_nblocks=2,mh_jscale=0.3,mh_drop=0.2);
