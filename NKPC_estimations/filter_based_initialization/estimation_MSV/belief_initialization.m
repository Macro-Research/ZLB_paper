function[alpha_init beta_init cc_init rr_init]=belief_initialization(parameters,dataset);
% clear;clc;close all;
...
alpha1=0*ones(3,1);beta1=0*ones(3,3);cc1=0*ones(3,2);rr=1*eye(4);
 load('ree_based_initials.mat');
beta1(:,3)=beta_init(:,3);
cc1=cc_init;
...
%alpha_init=alpha_tt(:,end);
alpha_init=zeros(3,1);
beta_init=beta_tt(:,end);
cc_init=cc_tt(:,:,end);
rr_init=rr;
