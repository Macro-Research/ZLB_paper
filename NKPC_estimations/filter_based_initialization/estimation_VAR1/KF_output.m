...
alpha1=0*ones(numVar,1);beta1=0*eye(numVar);rr=zeros(4,4);

[alpha_init,beta_init,rr_init]=belief_initialization(parameters,dataset);
alpha1(1:3)=alpha_init;
beta1(1:3,1:3)=beta_init;
rr=rr_init;
...
print(fig,'NKPC_filter_init_VAR1_regime','-dpdf');
...
fig_pos = fig.PaperPosition;
fig.PaperSize = [fig_pos(3) fig_pos(4)];
print(fig,'NKPC_filter_init_VAR1_regimeProb','-dpdf');
...
datetick('x','yyyy','keeplimits');
subplot(3,1,3);
plot(Date,alpha_tt(3,2:end),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');
fig = gcf;
fig.PaperPositionMode = 'auto'
fig_pos = fig.PaperPosition;
fig.PaperSize = [fig_pos(3) fig_pos(4)];
print(fig,'NKPC_filter_init_VAR1_alphas','-dpdf');

figure('Name','Learning: First-order autocorrelation');
subplot(3,3,1);
plot(Date,squeeze(beta_tt(1,1,2:end)),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');

subplot(3,3,2);
plot(Date,squeeze(beta_tt(1,2,2:end)),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');

subplot(3,3,3);
plot(Date,squeeze(beta_tt(1,3,2:end)),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');

subplot(3,3,4);
plot(Date,squeeze(beta_tt(2,1,2:end)),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');

subplot(3,3,5);
plot(Date,squeeze(beta_tt(2,2,2:end)),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');

subplot(3,3,6);
plot(Date,squeeze(beta_tt(2,3,2:end)),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');

subplot(3,3,7);
plot(Date,squeeze(beta_tt(3,1,2:end)),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');

subplot(3,3,8);
plot(Date,squeeze(beta_tt(3,2,2:end)),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');

subplot(3,3,9);
plot(Date,squeeze(beta_tt(3,3,2:end)),'lineWidth',3);
xlim([startDate endDate])
datetick('x','yyyy','keeplimits');
...
fig_pos = fig.PaperPosition;
fig.PaperSize = [fig_pos(3) fig_pos(4)];
print(fig,'NKPC_filter_init_VAR1_betas','-dpdf');