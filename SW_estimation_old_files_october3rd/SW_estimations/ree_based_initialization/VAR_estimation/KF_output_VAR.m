...
gamma2_2=AA2_inv*CC2*(eye(numVar)-beta1^2)*alpha1;gamma3_2=AA2_inv*DD2;
...
subplot(3,1,1);
plot(largest_eig1(2:end),'color','black');
hold on;plot(ones(length(largest_eig1(2:end)),1),'color','black');
title('Largest eigenvalue: normal regime');
subplot(3,1,2);
plot(largest_eig2(2:end),'color','black');
hold on;
plot(ones(length(largest_eig2(2:end)),1),'color','black');
title('Largest eigenvalue: zlb regime');
subplot(3,1,3);
plot(average_eig(2:end),'color','black');
hold on;
plot(ones(length(average_eig(2:end)),1),'color','black');
...
plot(pr_flag(2:end),'lineWidth',3);

fig = gcf;
fig.PaperPositionMode = 'auto'
fig_pos = fig.PaperPosition;
fig.PaperSize = [fig_pos(3) fig_pos(4)];
print(fig,'sw_ar1_pr_flag','-dpdf'); 
