...
numVar=24;numShocks=7;numEndo=17;numExo=7;
%fixed parameteters
...
[AA1 BB1 CC1 DD1 EE1 RHO1 FF1 GG1 E1 F1]=SW_sysmat_MSV_filter(parameters(1:end,1));
[AA2 BB2 CC2 DD2 EE2 RHO2 FF2 GG2 E2 F2]=SW_sysmat_MSV_filter(parameters(1:end,2));
...
dataset=dataset(first_obs:last_obs,:);l=7;N=length(dataset);burnIn=6;
T=size(dataset,1);numObs=7;
startDate=datenum('01-01-1966');
endDate = datenum('01-12-2016');
Date=linspace(startDate,endDate,T);
alpha_tt=0*ones(numEndo,1);
beta_tt=0.75*eye(numEndo);
cc_tt=zeros(numEndo,numExo);
...
S_fore11=zeros(numVar,1);
S_fore12=zeros(numVar,1);
S_fore21=zeros(numVar,1);
S_fore22=zeros(numVar,1);
P_fore11=zeros(numVar,numVar);
P_fore12=zeros(numVar,numVar);
P_fore21=zeros(numVar,numVar);
P_fore22=zeros(numVar,numVar);
v11=zeros(numObs,1);
v12=zeros(numObs,1);
v21=zeros(numObs,1);
v22=zeros(numObs,1);
Fe11=zeros(numObs,numObs);
Fe12=zeros(numObs,numObs);
Fe21=zeros(numObs,numObs);
Fe22=zeros(numObs,numObs);
S_upd11=zeros(numVar,1);
S_upd12=zeros(numVar,1);
S_upd21=zeros(numVar,1);
S_upd22=zeros(numVar,1);
P_upd11=zeros(numVar,numVar);
P_upd12=zeros(numVar,numVar);
P_upd21=zeros(numVar,numVar);
P_upd22=zeros(numVar,numVar);
...
% gamma2_1=AA1_inv*CC1*(eye(numVar)+beta_tt^2)*alpha_tt;
% gamma3_1=AA1_inv*DD1;%gamma4_1=AA1_inv*EE1;
% 
% gamma2_2=AA2_inv*CC2*(eye(numVar)+beta_tt^2)*alpha_tt;
