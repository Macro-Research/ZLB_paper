...
imp11(index,:,:,:)=impulse_response(gamma1_1,gamma3_1,periods);
imp22(index,:,:,:)=impulse_response(gamma1_2,gamma3_2,periods);
imp33(index,:,:,:)=impulse_response(gamma1_avg,gamma3_avg,periods);
end
...
index=0;
 figure('Name','Impulse Responses-VAR(1) Model');
for ii=1:2
    for jj=1:2
...
% plot3(refLine1',refLine2',imp11(:,:,ii,jj)','k','lineWidth',1);
% hold on;
% plot3(refLine1',refLine2',imp22(:,:,ii,jj)','color','green','lineStyle','--','lineWidth',1);
plot(imp11(10,:,ii,jj),'lineWidth',3,'color','blue');
hold on;
