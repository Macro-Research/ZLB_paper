function GM_simulation_var_dec(TT,RR,Q,mus,lgy,order)
...
% Generating variance decomposition

select = [pinf_pos;dy_pos;y_pos;dw_pos;w_pos];
var_dec = zeros(4,length(select),num_shock+1,smpl);
for t = 1:smpl
    T = TT(:,:,t);
    V = RR(:,:,t) * chol(Q);
    for j = 1:num_shock
        cV = V(:,j);
        % one quarter ahead variance
        tmp = cV * cV';
        tmp1 = diag(tmp);
        var_dec(1,:,j,t) = tmp1(select);
        % one year ahead variance
        tmp4 = diag(tmp + T * tmp * T' + T^2 * tmp * (T^2)' + T^3 * tmp * (T^3)');
        var_dec(2,:,j,t) = tmp4(select);
        % 10 years ahead variance
        tmp40 = tmp;
        for i = 1:39
            tmp40 = tmp40 + T^i * tmp * T'^i;
        end
        tmp40 = diag(tmp40);
        var_dec(3,:,j,t) = tmp40(select);
        % unconditional variance
        tmp_unc = diag(lyapunov_symm(T,tmp));
        var_dec(4,:,j,t) = tmp_unc(select);
    end
    % The things should sum to one over the 2nd dimension here
    var_dec(:,:,num_shock+1,t) = sum(var_dec(:,:,1:num_shock,t),3);        
end
