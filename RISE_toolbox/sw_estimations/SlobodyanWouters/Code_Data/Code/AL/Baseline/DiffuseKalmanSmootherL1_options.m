function [alphahat,etahat,a, aK, filteratt,bet] = DiffuseKalmanSmootherL1(T,R,Q,Pinf1,Pstar1,Y,trend,pp,mm,smpl,mf,gain)
% modified by M. Ratto:
...
% for a model with simple constant gain learning WITHOUT constant
...
bet = zeros(length(ys_list)+length(shock_list),length(yf_list),smpl);
...
TT(:,:,1) = T;
RR(:,:,1) = R;
% This is diffuse priors block, will never run under AL
while rank(Pinf(:,:,t+1),crit1) & t<smpl
    ...
    %-------------------------------------------------- 
    KwithoutTinf(:,:,t)	 	= Pinf(:,mf,t)*iFinf(:,:,t);
    filteratt(:,t)=a(:,t) + KwithoutTinf(:,:,t)*v(:,t);
    %-------------------------------------------------- 
    aK(1,:,t+1) 	 	= a(:,t+1);
    for jnk=2:nk,
...
% The smoothing block should work correctly
t = smpl+1;
while t>d+1 && t>2
	t = t-1;
