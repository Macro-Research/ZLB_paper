...
parameters(coef,2) crpi crr cry crdy sig_m czlb crb
...
% sticky price - wage economy
   ...
   r =  crr*(r(-1))+
                  crpi*(1-crr)*pinf +
                    ...
                    crdy*(output_gap-output_gap(-1))-
                    crb*(1-crr)*sig_b*eb  +
                    ms -czlb*(1-crr)*(conster-0.25/4/2+0*conster)             ;
          output_gap=y-cfc*a;
...
parameterization
	...
	cry(coef,1),       0.1,0.125,0.05,normal_pdf;
	crdy(coef,1),      0.2,0.125,0.05,normal_pdf;
    crb(coef,1),       0, 0,0.2,normal_pdf;
