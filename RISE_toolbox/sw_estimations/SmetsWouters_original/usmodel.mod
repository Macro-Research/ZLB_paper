...
// sticky price - wage economy
	      ...
	      w =  (1/(1+cbetabar*cgamma))*w(-1)
               ...
               + 1*sw ;
	      //r =  crpi*(1-crr)*pinf+cry*(1-crr)*(y-cfc*a)+crdy*(y-cfc*a-y(-1)+cfc*a(-1))+crr*r(-1)+ms;
...
//estimation(optim=('MaxIter',500),datafile=usmodel_data_SW_data_april2009,nograph,mode_compute=1,first_obs=71,presample=4,kalman_algo=1,lik_init=2,prefilter=0,mh_replic=0,mh_nblocks=2,mh_jscale=0.3,mh_drop=0.2);
 ...
 //  estimation(optim=('MaxIter',200),datafile=usmodel_data,mode_compute=1,mode_file=usmodel_mode,first_obs=71,presample=4,lik_init=2,prefilter=0,mh_replic=0,mh_nblocks=2,mh_jscale=0.20,mh_drop=0.2);
