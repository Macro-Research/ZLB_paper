   function[alpha_init,beta_init,rr_init]=simulation_AR1(parameters)
...
X(:,t) = (regime(t))* A1_inv * (( B1+C1*beta1^2)*X(:,t-1)+C1*(alpha1+beta1*alpha1)+D1*errors1(:,t))+...
    (1-regime(t))* A2_inv * (( B2+C2*beta1^2)*X(:,t-1)+C2*(alpha1+beta1*alpha1)+D2*errors2(:,t));
        ...
        for jj=1:2
    [alpha1(jj) beta1(jj,jj) r1(:,:,jj) largestEig(t) pr_flag(t)] =...
         msv_learning(X(jj,t),[1,X(jj,t-1)]',...
      alpha1(jj),beta1(jj,jj),r1(:,:,jj),gain);
