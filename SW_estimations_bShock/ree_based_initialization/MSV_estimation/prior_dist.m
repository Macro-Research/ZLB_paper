...
prior28= exp(lpdfig1(param(28),s,nu));
prior29= exp(lpdfig1(param(29),s,nu));
...
prior35= exp(lpdfig1(param(35),s,nu));

mu=0.03;sigma2=0.01^2;b  = sigma2/mu;a  = mu/b;
prior33=gampdf(param(33),a,b);%policy shock at the zlb regime


 mu=0.035;sigma2=0.015^2;b  = sigma2/mu;a  = mu/b;
prior36=gampdf(param(36),a,b);%gain

 mu=0.1;sigma2=0.05^2;a = (1-mu)*mu*mu/sigma2-mu;b = a*(1/mu-1);
 prior37=betapdf(param(37),a,b);%1-p_11
  mu=0.3;sigma2=0.1^2;a = (1-mu)*mu*mu/sigma2-mu;b = a*(1/mu-1);
 prior38=betapdf(param(38),a,b);%1-p_22
  prior39=normpdf(param(39),0.05,0.25);%rbar_zlb
...
prior=prior1*prior2*prior3*prior4*prior5*prior6*prior7*prior8*...
    ...
    prior32*prior33*prior34*prior35*prior36*prior37*prior38*prior39;
prior=-log(prior);
end